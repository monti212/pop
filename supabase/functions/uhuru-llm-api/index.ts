...
import {
  ...
  normalizeStream,
} from '../_shared/stream_metering.ts';
...
async function fetchRelevantKnowledgeBase(
  ...
  try {
    ...
    // else — there is no .catch() on the builder. Calling it threw a synchronous
    // TypeError that the catch below swallowed, so every request that retrieved
    // chunks silently returned an EMPTY knowledge section. Promise.resolve() adapts
    // the thenable into a real promise, which does have .catch().
    if (chunkIds.length > 0) {
      void Promise.resolve(
        supabaseClient.rpc('update_chunk_usage_stats', { p_chunk_ids: chunkIds })
      )
        .then(({ error }: any) => {
          if (error) console.warn('Failed to update chunk usage stats:', error.message);
        })
        .catch((err: any) => {
...
Respond with ONLY valid JSON, no markdown, no explanation:
{
  ...
  try {
    ...
    return new Response(normalizeStream(up.body, {
      // RETURN the promise — normalizeStream settles it before closing the stream.
      onComplete: (usage) => {
        if (!requestUser?.id) return;
        return recordTokenUsage(supabase, {
          userId: requestUser.id,
