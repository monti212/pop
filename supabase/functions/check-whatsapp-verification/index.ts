...
async function checkTwilioVerification(phoneNumber: string, code: string): Promise<{ success: boolean; valid?: boolean; error?: string }> {
  try {
    if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN || !TWILIO_VERIFY_SERVICE_SID) {
      throw new Error('WhatsApp service configuration is missing');
    }
...
async function findUserByPhone(phoneNumber: string): Promise<{ found: boolean; userId?: string; error?: string }> {
  try {
    ...
    const userWithPhone = authUsers.users?.find(u => u.phone === phoneNumber || u.user_metadata?.phone === phoneNumber);
...
async function createUserWithPhone(phoneNumber: string, name?: string): Promise<{ success: boolean; userId?: string; error?: string }> {
  try {
    ...
    const { error: authError } = await supabase.auth.admin.createUser({
      ...
      user_metadata: {
        phone: phoneNumber,
        via_whatsapp_verification: true,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!verification) {
      return new Response(JSON.stringify({ error: 'No active verification found. Please request a new code.' }), {
        status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (currentUserId) {
      ...
      await supabase
        .from('user_profiles')
        .update({ phone_number: normalizedPhone, display_name: name || undefined })
        .eq('id', currentUserId);
    ...
    return new Response(JSON.stringify({
      ...
      session: {
        ...
        token_type: 'bearer',
        expires_in: 3600,
        user: { id: userId, phone: normalizedPhone, user_metadata: authUser.user_metadata || {} }
      },
