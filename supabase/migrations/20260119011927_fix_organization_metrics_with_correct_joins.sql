/*
  # Fix Organization Metrics Function with Correct Table Joins
  
  1. Changes
    - Fix function to join messages through conversations to get user_id
    - Use correct column relationships
    
  2. Purpose
    - Properly calculate active users by joining messages -> conversations -> user_id
*/
...
BEGIN
  ...
  SELECT COUNT(DISTINCT c.user_id) INTO v_active_today
  FROM messages m
  JOIN conversations c ON m.conversation_id = c.id
  WHERE m.created_at >= CURRENT_DATE;

  -- Count active users this week
  SELECT COUNT(DISTINCT c.user_id) INTO v_active_week
  FROM messages m
  JOIN conversations c ON m.conversation_id = c.id
  WHERE m.created_at >= CURRENT_DATE - INTERVAL '7 days';

  -- Count active users this month
  SELECT COUNT(DISTINCT c.user_id) INTO v_active_month
  FROM messages m
  JOIN conversations c ON m.conversation_id = c.id
